 ...
 * @version 1
 * 
 * OrderList models a list of Orders for a particular file.
 * The input file has a list of orders. The input file is read and each line, which is an order is used to create an order object.
 * All the Order objects corresponding to an input file is kept in an array of Orders.
 * 
 * 
 */
import java.io.*;
...
{
    // Parallel arrays used for processing
    ...
    private Order[] orderList; //Content of file is stored in memory as an array of Order objects
    private int numberOfOrders; //Number of orders
    

    /**
     * Constructor for objects of class OrderList
     * Constructor reads the input file, creates Order objects corresponding to each line in input file
     ...
     * It accepts two arguments. If output to file is required (option 3), writefile is true
     * and out is the BufferedWriter object to write to.
     * Else (option 2), writeFile is false and out is null and output is written only to console.
     */
        ...
        for(int i = 0; i < numberOfOrders; i++)
        {
            if (!nextLaunchSameDay)
            {
                ...
                if(orderList[i].getLaunchVehicle().equals("BFR"))
                {
                    ...
                    //If option 3, write to file also
                    if(writeFile)
                    {
                        for(int j = 0; j < stringArray.length; j++)
                        {
                ...
                {
                    //Else (it is not BFR), use launchpad LC40
            ...
            {
                //This is afternoon launch. Use launchpad stored in nextLaunchPad variable;
     ...
     * Returns String Array consisting of plan statements
     */
    private String[] earlyLaunch(Order order, String launchPad)
    {
        //This method stores all plan statements of AM launches in ArrayList and then returns it back as an array of Strings
    ...
    {
        ////This method stores all plan statements of PM launches in ArrayList and then returns it back as an array of Strings
        ArrayList<String> arrayList = new ArrayList();
        String date = order.getDateFormatted();
        String launchVehicle = order.getLaunchVehicle();
        String launchID = order.getLaunchID();
